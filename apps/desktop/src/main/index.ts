...
// Initialize Sentry first to catch all possible errors
...
if (process.defaultApp) {
  if (process.argv.length >= 2) {
    app.setAsDefaultProtocolClient('viclip', process.execPath, [path.resolve(process.argv[1])]);
  }
} else {
...
app.whenReady().then(() => {
  ...
  app.setLoginItemSettings({
    ...
    args: ['--processStart', 'app.exe', '--processArgv', 'ViClip.exe', '--hidden']
  });
