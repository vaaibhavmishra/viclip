 ...
 */
export async function checkUpdateWithRetry(maxRetries = 3, retryDelay = 60000): Promise<void> {
  let attempt = 0;
  ...
  while (attempt < maxRetries) {
    try {
      ...
      attempt++;
      log.warn(`Update check failed (attempt ${attempt}/${maxRetries})`, error as Error);
